...
// ##################################
if (TelValor === "") {
    setErrorInput (Tel, "El campo nombre no puede estar vacio.");
} else {
    setSuccessInput(Tel);
}
...
if(claveValor ===""){
    ...
    setSuccessInput(clave) && validarCorreo(claveValor);
}

// ##################################
if (clave2Valor === ""){
    setErrorInput(clave2, "El campo clave no puede estar vacio. ");
...
function validarCorreo(correo) {
    let regular_expressions = /^(([^<>()\[\]\\.,;:\s@"]+(\.[^<>()\[\]\\.,;:\s@"]+)*)|(".+"))@((\[[0-9]{1,3}\.[0-9]{1,3}\])|(([a-zA-Z\-0-9]+\.)+[a-zA-Z] {2,}))$/;
